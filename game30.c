// =============================================================================
//   VirconDoom "game30" ROM -- CATCH-UP TIMING EXPERIMENT (fork of game.c).
//
//   Difference from game.c: the playsim is slaved to wall-clock via
//   get_frame_counter() -- the SAME clock the synth music already uses -- so the
//   two can never drift. Each iteration runs however many 30 Hz tics have elapsed
//   in real time (0..SIM_MAXTICS), then renders once. Under load the sim keeps
//   full game speed (framerate drops instead of the whole game going slow-motion),
//   and the music stays in sync structurally. This is the DOOM-native "run the
//   updates N times per screen draw" approach (upstream TryRunTics); DOOM is
//   fixed-timestep, so the "scale delta time" variant does not apply. See MUSIC.md
//   and the Discord discussion that motivated this.
//
...
//   Verify: with the debug HUD open, under heavy load TIC rises above 1 while the
//   game keeps normal speed (leveltime tracks wall-clock) and music stays synced.
// =============================================================================
