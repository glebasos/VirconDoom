// =============================================================================
//   VirconDoom "gameauto" ROM -- CATCH-UP + ADAPTIVE (AUTOMATIC) FRAMESKIP
//   (fork of gameskip.c; the render-skip is demand-driven instead of a fixed 1-in-N).
//
//   The playsim is slaved to wall-clock exactly like game30, so it never drifts
//   from the music. The difference from gameskip: instead of unconditionally
//   rendering 1 of every (FRAMESKIP+1) iterations, this ROM renders EVERY frame
//   while it is keeping up (owed <= 1) and only starts SKIPPING draws once the sim
//   falls behind (owed >= 2 => rendering can't sustain 30 Hz). It skips exactly as
//   much as the overrun demands, but never more than MAXSKIP draws in a row, so the
//   picture can't freeze. This is the "automatic frameskipping" the Vircon32 author
//   described: detect how many frames the update overran and drop that many draws.
//   Best of both -- crisp at full speed, degrades to lower visual framerate (not
//   slow-motion, not music desync) only under load.
//
...
//   Verify: debug HUD SKP stays 0 in easy areas and rises (up to MAXSKIP) only in
//   heavy ones, while game speed and music stay steady the whole time.
// =============================================================================
...
// Max draws to skip in a row before forcing a render, so the picture can't freeze
// under sustained overload (a floor of 1 rendered frame per MAXSKIP+1). Skipping is
// otherwise demand-driven: it happens only while the sim is behind (owed >= 2).
...
void main()
{
    ...
    int skipped = 0;             // draws skipped in a row since the last render
    ...
    while( true )
    {
        ...
        // ---- adaptive frameskip decision. The logic (input + sim tics) above
        // already ran; owed is how many tics real time demanded this iteration.
        // owed <= 1 means we're keeping up -> render. owed >= 2 means rendering is
        // too slow to hold 30 Hz -> skip this draw to give the CPU slack, but force
        // a render once we've skipped MAXSKIP in a row so the picture can't freeze.
        bool doRender = ( owed <= 1 || skipped >= MAXSKIP );
